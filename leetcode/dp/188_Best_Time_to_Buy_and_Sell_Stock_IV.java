...
Approach:
- Use recursion to explore all possible transactions (buy/sell/skip) with at most k transactions allowed.
...
class Solution {
    ...
    public int maxProfit(int k, int[] prices) {
        int n=prices.length;
        int [][][]t = new int[n][k+1][2];
        for(int [][]arr: t)
            for(int []a: arr)
                Arrays.fill(a, -1);
        return solve(0, 1, k, n, prices, t);
    }
}
...
class Solution{
    public int maxProfit(int k, int[] prices) {
        int n=prices.length;
            int [][][]dp = new int[n+1][k+1][2]; //where n is the number of days, 2 is for buy/sell state, and 3 is for the max 2 transactions (0, 1, 2).
